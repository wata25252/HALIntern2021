...
namespace RayFire
{
    ...
    public class RayfireRigidRoot : MonoBehaviour
    {
        ...
        [NonSerialized]   public List<RFShard>           meshRootShards;
        [NonSerialized]   public List<RFShard>           rigidRootShards;
        ...
        void SetColliders()
        {
            ...
            {
                for (int i = 0; i < rigidRootShards.Count; i++)
                    RFPhysic.SetRigidRootCollider (physics, rigidRootShards[i]);
                for (int i = 0; i < meshRootShards.Count; i++)
                    RFPhysic.SetRigidRootCollider (meshRootShards[i].rigid.physics, meshRootShards[i]);
            }
        }
